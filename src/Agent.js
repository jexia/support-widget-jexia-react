...
class Agent extends Component {
    ...
    componentDidMount() {
        ums.signIn(agent_cred).subscribe(user => {
            // if signIn for agent is good, we can fetch data...
        ...
        return (<div className='container-fluid' >
            <div className='row' >
                ...
                <div className="col-md-8 h-100pr border rounded" >
                        <div className='row' >
                            <div className='col-lg-4 col-xs-12 bg-light'
                                style={
                                    { height: 658 }
                                } >
                                    ...
                                    <div className='row ml-0 mr-0 h-75 bg-white border rounded'
                                        ...
                                        <div className='row p-3 bg-white' >
                                            ...
                                            >
                                                    ...
                                                    <form className="row m-0 p-0 w-100"
...
class ChatBox extends Component {
        ...
        if (chatIsLoading) {
            return ( <div className='col-xl-12 my-auto text-center' >
                        <MDSpinner size='72' />
                    </div>)
        } else {
            return ( <div className='col-xl-12' > {
                        ...
                        .map(chat =>
                            <div key={chat.id} className="message" >
                             <div className={`${chat.is_user ? 'balon1' : 'balon2'} p-3 m-1`} > {chat.txt} </div> 
                             </div> )
                } 
                </div>
            )
...
class CustomerList extends Component {
        ...
        if (customerIsLoading) {
            ...
            return ( <ul className="list-group list-group-flush w-100" > {
                        customers
                            .map(customer =>
                                <li key={customer.id}
                                className={`list-group-item ${customer.session_id === selectedCustomer ? 'active' : ''}`}
                                onClick={
                                    () => this.props.selectCustomer(customer.session_id)
                                } > Customer {customer.session_id} </li>)
                    } </ul>
                )
            }
        }
